...
for x in range(len(all_img_name)):
    ...
    print('-------------------')
    query = all_img_name[x].split('/')[4] + '/' +all_img_name[x].split('/')[5]+'.jpg'
    print(query)
    print(rank_img[1])
    ...
    print('-------------------')
    dir2 = '../result/' + all_img_name[x].split('/')[5]
    if not os.path.isdir(dir2):
        os.mkdir(dir2)
    im = cv2.imread(dir+all_img_name[x].split('/')[4]+'/'+all_img_name[x].split('/')[5]+'.jpg')
    cv2.imwrite(dir2 + '/' +'query.jpg', im)
    for j in range(1, 6):
        result = dir+rank_img[j].split('/')[4] + '/' + rank_img[j].split('/')[5] + '.jpg'
        im = cv2.imread(result)
        cv2.imwrite(dir2 +'/'+ rank_img[j].split('/')[5] + '.jpg', im)
